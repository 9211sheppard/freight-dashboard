...
async function fetchStats() {
  try {
    ...
    // Stats are optional in this view.
  }
}
...
async function fetchVerifyStats() {
  try {
    ...
    // Verify stats are optional in this view.
  }
}
  ...
  refreshBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        if (window.sToast) window.sToast(`Refreshed — ${data.row_count.toLocaleString()} contacts`);
      }
    } catch (_) {
...
if (csvFileInput) {
  csvFileInput.addEventListener('change', async () => {
    ...
    try {
      ...
      if (data.ok) {
        fetchResults(); fetchStats();
        if (window.sToast) window.sToast(`${data.network} imported — ${data.imported.toLocaleString()} contacts`);
      }
    } catch (_) {
      // Import feedback is best-effort.
    } finally {
