...
TMS_DB = os.path.join(os.path.dirname(__file__), "tms.db")
CONTACTS_DB = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "contacts.db")
DEFAULT_TENANT_REGION = "ca-central"
DEFAULT_TENANT_SESSION_TIMEOUT_MINUTES = 30
POD_UPLOAD_DIR = os.path.join(os.path.dirname(__file__), "uploads", "pods")
...
def _upsert_portal_token(conn, portal_token):
    ...
    if existing:
        conn.execute(
            ...
            SET customer_name = ?, email = ?, shipment_refs = ?
            WHERE token = ?
            """,
            params,
        )
        ...
        conn.execute(
            """
            INSERT INTO portal_tokens (token, customer_name, email, shipment_refs)
            VALUES (?, ?, ?, ?)
            """,
            (
                token,
...
def list_api_keys():
    ...
    try:
        rows = conn.execute(
            """
            SELECT key, customer_name, permissions, created_at, last_used
            FROM api_keys
...
def create_api_key(customer_name, permissions):
    ...
    try:
        ...
        while not api_key_value:
            ...
            existing = conn.execute("SELECT key FROM api_keys WHERE key = ?", (candidate,)).fetchone()
            if not existing:
                api_key_value = candidate

        conn.execute(
            """
            INSERT INTO api_keys (key, customer_name, permissions)
            VALUES (?, ?, ?)
            """,
            (
                api_key_value,
        ...
        conn.commit()
        return get_api_key(api_key_value)
    finally:
        conn.close()
...
def revoke_api_key(api_key_value):
    ...
    try:
        ...
        conn.execute("DELETE FROM api_keys WHERE key = ?", (clean_api_key,))
        conn.commit()
...
def save_portal_token(token, customer_name, email="", shipment_refs=None):
    init_tms_db()
    conn = get_db()
    try:
...
def get_portal_token(token):
    ...
    try:
        ...
        return _portal_row_to_dict(row)
    finally:
        conn.close()
...
def save_carrier(carrier_id=None, *, name, scac="", dot_number="", country="", contact_email="", contact_phone="", active=1):
    init_tms_db()
    clean_name = _normalize_text(name)
    clean_scac = _normalize_scac(scac)
    ...
    try:
        ...
        if carrier_id:
            ...
            conn.execute(
                """
                UPDATE tms_carriers
                SET name = ?, scac = ?, dot_number = ?, country = ?, contact_email = ?, contact_phone = ?,
            ...
            cursor = conn.execute(
                ...
                INSERT INTO tms_carriers
                    (name, scac, dot_number, country, contact_email, contact_phone, active, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                """,
                (
                    clean_name,
...
def save_driver(driver_id=None, *, name, license_number, phone="", country="", status="Active"):
    init_tms_db()
    clean_name = _normalize_text(name)
    ...
    try:
        ...
        if driver_id:
            ...
            conn.execute(
                """
                UPDATE drivers
                SET name = ?, license_number = ?, phone = ?, country = ?, status = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
                """,
                (clean_name, clean_license, clean_phone, clean_country, clean_status, driver_id),
            )
            saved_id = driver_id
        else:
            cursor = conn.execute(
                """
                INSERT INTO drivers
                    (name, license_number, phone, country, status, checkin_token, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                """,
                (
                    clean_name,
        ...
        if vehicle_id:
            ...
            conn.execute(
                ...
                SET truck_number = ?, vehicle_type = ?, capacity_weight = ?, capacity_cbm = ?,
                    country = ?, status = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
                """,
                (
                    clean_truck_number,
            ...
            cursor = conn.execute(
                ...
                INSERT INTO vehicles
                    (truck_number, vehicle_type, capacity_weight, capacity_cbm, country, status, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                """,
                (
                    clean_truck_number,
