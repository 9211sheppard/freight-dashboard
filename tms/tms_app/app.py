...
def _is_production_mode(app):
    return app.config.get("TMS_ENV", "development").strip().lower() == "production"
...
def _validate_startup_config(app):
    ...
    if not base_host or base_host not in app.config.get("ALLOWED_HOSTS", set()):
    ...
    for email_key, password_key, password_hash_key in required_seeds:
...
def _trusted_external_url(app, endpoint, **values):
    relative_url = url_for(endpoint, _external=False, **values)
    base_url = str(app.config.get("BASE_URL", "") or "").strip()
    if base_url:
        return urljoin(base_url.rstrip("/") + "/", relative_url.lstrip("/"))
    if app.config.get("TESTING"):
        return urljoin((request.host_url or "http://127.0.0.1/"), relative_url.lstrip("/"))
    return relative_url
...
def create_app(test_config=None):
    ...
    app.config.from_mapping(
        ...
        TMS_ADMIN_EMAIL=os.getenv("TMS_ADMIN_EMAIL", ""),
        TMS_ADMIN_PASSWORD=os.getenv("TMS_ADMIN_PASSWORD", ""),
        TMS_ADMIN_PASSWORD_HASH=os.getenv("TMS_ADMIN_PASSWORD_HASH", ""),
        TMS_ADMIN_NAME=os.getenv("TMS_ADMIN_NAME", "Primary Admin"),
        TMS_DISPATCHER_EMAIL=os.getenv("TMS_DISPATCHER_EMAIL", ""),
        TMS_DISPATCHER_PASSWORD=os.getenv("TMS_DISPATCHER_PASSWORD", ""),
        ...
        TMS_VIEWER_EMAIL=os.getenv("TMS_VIEWER_EMAIL", ""),
        TMS_VIEWER_PASSWORD=os.getenv("TMS_VIEWER_PASSWORD", ""),
    ...
    def sso_login():
        ...
        context = build_saml_stub_context(
            tenant,
            issuer=_trusted_external_url(app, "saml_acs"),
            acs_url=_trusted_external_url(app, "saml_acs"),
        )
