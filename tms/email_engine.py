...
try:
    from config import BASE_URL as CONFIG_BASE_URL, DB_PATH
except Exception:
    CONFIG_BASE_URL = ""
    DB_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "contacts.db")

DEFAULT_DB_PATH = os.getenv("TMS_CONTACTS_DB_PATH") or DB_PATH

try:
    from .tms_integrations import decrypt_key, encrypt_key
except Exception:
    from tms_integrations import decrypt_key, encrypt_key
...
PROVIDER_SECRET_PREFIX = "enc:v1:"


def _configured_base_url() -> str:
    configured = os.getenv("BASE_URL") or CONFIG_BASE_URL
    return str(configured or "").rstrip("/")


DEFAULT_TRACKING_BASE_URL = (
    f"{_configured_base_url()}/tms/email" if _configured_base_url() else ""
)
...
def _encode_provider_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in PROVIDER_SECRET_KEYS:
        ...
        if clean_value.startswith(PROVIDER_SECRET_PREFIX):
            continue
        encrypted = encrypt_key(clean_value)
        payload[key] = f"{PROVIDER_SECRET_PREFIX}{encrypted}" if encrypted else ""
    return payload


def _decode_provider_config(config: dict[str, Any]) -> dict[str, Any]:
    payload = dict(config or {})
    for key in PROVIDER_SECRET_KEYS:
        value = payload.get(key)
        if not value:
            continue
        clean_value = str(value)
        if not clean_value.startswith(PROVIDER_SECRET_PREFIX):
            continue
        payload[key] = decrypt_key(clean_value[len(PROVIDER_SECRET_PREFIX):])
    return payload


def _provider_config_needs_migration(config: dict[str, Any]) -> bool:
    for key in PROVIDER_SECRET_KEYS:
        value = config.get(key)
        if value and not str(value).startswith(PROVIDER_SECRET_PREFIX):
            return True
    return False
...
class EmailEngine:
    def __init__(
        ...
        self.tracking_base_url = str(tracking_base_url or "").rstrip("/")
    ...
    def init_db(self) -> None:
        with self._connect() as conn:
            ...
            rows = conn.execute(
                "SELECT provider, config_json FROM email_provider_configs"
            ).fetchall()
            now = _utcnow_iso()
            for row in rows:
                config = _json_loads(row["config_json"])
                if not _provider_config_needs_migration(config):
                    continue
                conn.execute(
                    """
                    ...
                    WHERE provider = ?
                    """,
                    (json.dumps(_encode_provider_config(config), sort_keys=True), now, row["provider"]),
                )
            conn.commit()
    ...
    def save_provider_config(self, provider: str, config: dict[str, Any]) -> dict[str, Any]:
        ...
        stored_payload = _encode_provider_config(payload)
        now = _utcnow_iso()
        with self._connect() as conn:
            conn.execute(
                """
                ...
                ON CONFLICT(provider) DO UPDATE SET
                    ...
                    updated_at=excluded.updated_at
                """,
                (provider, _json_dumps(stored_payload), now),
            )
            conn.commit()
        return _decode_provider_config(stored_payload)
    ...
    def get_provider_config(self, provider: str, required: bool = True) -> dict[str, Any]:
        ...
        if not row:
            ...
            return {}
        config = _json_loads(row["config_json"])
        decoded = _decode_provider_config(config)
        if _provider_config_needs_migration(config):
            self.save_provider_config(provider, decoded)
        return decoded
    ...
    def instrument_html(
        ...
        click_base = self.get_setting("tracking_base_url", self.tracking_base_url).rstrip("/")
        if track_clicks and click_base:
            instrumented = self._rewrite_links(instrumented, tracking_id)
        if track_open:
            open_url = self.build_open_url(tracking_id)
            if not open_url:
                return instrumented
            pixel = (
                f'<img src="{html.escape(open_url, quote=True)}" '
