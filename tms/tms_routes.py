...
try:
    from .tms_docs import generate_bol, generate_customer_invoice, generate_invoice, generate_packing_list, generate_pod
...
@tms.route("/loads", methods=["GET", "POST"])
def loads():
    form_values = {
        "carrier_id": (request.form.get("carrier_id") or "").strip(),
        "status": (request.form.get("status") or LOAD_STATUSES[0]).strip(),
        "shipment_refs": request.form.getlist("shipment_refs"),
    }
    form_error = None
    status_code = 200
    ...
    if request.method == "POST":
        try:
            load = create_load(
                shipment_refs=form_values["shipment_refs"],
                carrier_id=form_values["carrier_id"],
                status=form_values["status"],
            )
        except ValueError as exc:
            form_error = str(exc)
            ...
            flash(f"Load {load['load_ref']} created.", "success")
            return redirect(url_for("tms.view_load", load_ref=load["load_ref"]))

    context = _get_load_board_context(form_values=form_values, form_error=form_error)
    return render_template("tms/loads.html", **context), status_code
...
# -- Integrations --------------------------------------------------------------
from .tms_integrations import INTEGRATIONS, get_by_category, get_categories

@tms.route('/integrations')
def integrations():
    conn = get_db()
    saved = {r['id']: dict(r) for r in conn.execute('SELECT * FROM tms_integrations').fetchall()}
    conn.close()
    by_cat = get_by_category()
    for cat, items in by_cat.items():
        for item in items:
            s = saved.get(item['id'], {})
            item['status'] = s.get('status', 'disconnected')
            item['connected_at'] = s.get('connected_at', '')
            item['last_sync'] = s.get('last_sync', '')
    categories = get_categories()
    total_connected = sum(1 for s in saved.values() if s.get('status') == 'connected')
    return render_template('tms/integrations.html',
        by_cat=by_cat, categories=categories,
        total_connected=total_connected, total=len(INTEGRATIONS))

@tms.route('/integrations/<iid>/connect', methods=['POST'])
def connect_integration(iid):
    if iid not in INTEGRATIONS:
        return jsonify({'ok': False, 'error': 'Unknown integration'}), 404
    intg = INTEGRATIONS[iid]
    creds = {}
    for f in intg.get('fields', []):
    ...
    conn = get_db()
    conn.execute('''INSERT INTO tms_integrations (id, name, category, status, credentials_json, connected_at)
        VALUES (?, ?, ?, 'connected', ?, ?)
        ON CONFLICT(id) DO UPDATE SET status='connected', credentials_json=excluded.credentials_json, connected_at=excluded.connected_at, error_msg=NULL''',
        (iid, intg['name'], intg['category'], json.dumps(creds), datetime.utcnow().isoformat()))
    conn.commit()
    conn.close()
    return jsonify({'ok': True, 'message': f'{intg["name"]} connected successfully.'})

@tms.route('/integrations/<iid>/disconnect', methods=['POST'])
def disconnect_integration(iid):
    conn = get_db()
    conn.execute('UPDATE tms_integrations SET status=?, credentials_json=?, connected_at=NULL WHERE id=?',
        ('disconnected', '{}', iid))
    conn.commit()
    conn.close()
    return jsonify({'ok': True})

@tms.route('/integrations/<iid>/sync', methods=['POST'])
def sync_integration(iid):
    # Stub: in production this triggers the actual API call per integration
    conn = get_db()
    conn.execute('UPDATE tms_integrations SET last_sync=? WHERE id=?',
        (datetime.utcnow().isoformat(), iid))
    conn.commit()
    conn.close()
    return jsonify({'ok': True, 'message': 'Sync triggered. Data will update within 60 seconds.'})
