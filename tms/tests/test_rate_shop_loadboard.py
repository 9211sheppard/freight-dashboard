...
class RateShopLoadboardTests(unittest.TestCase):
    ...
    def test_loadboard_interest_rate_limit_blocks_tender_creation(self):
        with mock.patch("tms.tms_routes.LOADBOARD_INTEREST_RATE_LIMITER.allow", return_value=False):
        ...
        self.assertTrue(response.location.endswith("/loadboard"))
        with tms_db.get_db() as conn:
            tender = conn.execute("SELECT id FROM tenders").fetchone()
        self.assertIsNone(tender)

    def test_public_order_submit_rate_limit_blocks_order_creation(self):
        with mock.patch("tms.tms_routes.ORDER_SUBMIT_RATE_LIMITER.allow", return_value=False):
            response = self.client.post(
                "/tms/orders/submit",
                data={
                    ...
                    "cargo_description": "Dry groceries",
                    "service_type": "FTL",
        ...
        self.assertIn(b"Too many order submissions", response.data)
        with tms_db.get_db() as conn:
            order = conn.execute("SELECT order_ref FROM customer_orders").fetchone()
