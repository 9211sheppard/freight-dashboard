...
class EmailEngineTests(unittest.TestCase):
    ...
    def test_missing_tracking_base_url_fails_closed(self):
        engine = email_engine.EmailEngine(
            db_path=Path(self.tempdir.name) / "no-tracking.db",
            template_dir=self.template_dir,
            tracking_base_url="",
        )
        self.assertEqual(engine.build_open_url("track-1"), "")
        self.assertEqual(engine.build_click_url("track-1", "https://example.com"), "")
        instrumented = engine.instrument_html(
            '<html><body><a href="https://example.com/quote">Quote</a></body></html>',
            "track-1",
        )
        self.assertNotIn("/open/", instrumented)
        self.assertNotIn("/click/", instrumented)

    def test_provider_config_secrets_are_encrypted_at_rest(self):
        self.engine.connect_gmail(
            client_id="gmail-client",
            client_secret="gmail-secret",
            refresh_token="refresh-token",
        ...
        self.assertTrue(stored["client_secret"].startswith("enc:v1:"))
        self.assertTrue(stored["refresh_token"].startswith("enc:v1:"))
        self.assertTrue(stored["access_token"].startswith("enc:v1:"))
        self.assertNotIn("gmail-secret", stored["client_secret"])
        self.assertNotIn("refresh-token", stored["refresh_token"])
        self.assertNotIn("access-token", stored["access_token"])

        config = self.engine.get_provider_config("gmail")
        self.assertEqual(config["client_secret"], "gmail-secret")
