...
class DriverFleetRouteTests(unittest.TestCase):
    ...
    def test_driver_reply_requires_driver_token(self):
        driver = self._create_driver(name="Riley Stone", license_number="LIC-888")

        forbidden = self.client.post(
            f"/tms/drivers/{driver['id']}/messages/reply",
            json={"message": "Running 15 minutes behind."},
        )
        ...
        allowed = self.client.post(
            f"/tms/drivers/{driver['id']}/messages/reply",
            json={
                "driver_token": driver["checkin_token"],
                "message": "Running 15 minutes behind.",
            },
        )
        ...
        try:
            message = conn.execute(
                "SELECT direction, sent_by, message FROM driver_messages WHERE driver_id = ? ORDER BY id DESC LIMIT 1",
                (driver["id"],),
            ).fetchone()
        finally:
            conn.close()
        ...
        self.assertEqual(message["direction"], "from_driver")
        self.assertEqual(message["sent_by"], "Driver")
        self.assertEqual(message["message"], "Running 15 minutes behind.")
